...
<tr>
    ...
    $qq="SELECT *,student.login_id as slid,parents.login_id as plid,CONCAT(`student`.`first_name`,' ',`student`.`last_name`)AS stname,CONCAT(`parents`.`first_name`,' ',`parents`.`last_name`) AS pname,`student`.`phone` AS sphone,`student`.`email` AS semail FROM `student` INNER JOIN `parents` USING(`parent_id`) INNER JOIN `batch` USING(`batch_id`)";
    $result=select($qq);
	foreach($result as $rows)
	{
		...
		<tr>
            ...
            <td><a class="btn btn-primary btn-sm" href="teachermessages.php?selectedid=<?php echo $rows['plid']; ?>">Message To Student</a></td>
                    <td><a class="btn btn-primary btn-sm" href="teachermessages.php?selectedid=<?php echo $rows['slid']; ?>">Message To Parent</a></td>
